...
if (isset($_POST["submit"])) {
    ...
    if (in_array($_FILES["file"]["type"], $allowedFileType)) {
        ...
        for ($i = 0; $i <= $sheetCount; $i ++) {
			...
			$branch = "";
            if (isset($spreadSheetAry[$i][0])) {
                $branch = mysqli_real_escape_string($conn, $spreadSheetAry[$i][6]);
			}
			$year = "";
            if (isset($spreadSheetAry[$i][0])) {
                $year = mysqli_real_escape_string($conn, $spreadSheetAry[$i][7]);
			}
			$password=$phone;

            if (! empty($name) || ! empty($email) || ! empty($dob) || ! empty($phone) || ! empty($addr) || ! empty($branch) || ! empty($year)) {
                $query = "insert into user(id,name,email,dob,phone,addr,branch,adm,password) values(?,?,?,?,?,?,?,?,?)";
                $paramType = "sssssssss";
                $paramArray = array(
                    $id,
					$email,
